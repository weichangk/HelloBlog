...
// builder.Services.AddAutoDependencyInjection();// 自动注入（和下方使用 Autofac 注入的 builder.RegisterModule<AutofacModule>(); 二选一即可）
...
builder.Host.ConfigureContainer<ContainerBuilder>(container => {
    ...
    // 使用 AspectCore 加入动态代理（拦截器）注：切记 Autofac.Extensions.DependencyInjection 的 nuget 包必须是6.0版本？否则会出异常！
...
#region cookie 全局设置
// 设置认证 cookie 名称、过期时间、是否允许客户端读取
builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme).AddCookie(options =>
{
    options.Cookie.Name = "appsoft";// cookie 名称
...
#region 解决 Ubuntu Nginx 代理不能获取IP问题
